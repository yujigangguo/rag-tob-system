...
def create_kb(db: Session, user: User, req: KnowledgeBaseCreate) -> KnowledgeBase:
    """创建知识库:仅管理员;dept_admin 只能在本部门创建。"""
    ...
    db.commit()
    db.refresh(kb)
    logger.info("创建知识库: id=%s name=%s 部门=%s 检索方式=%s", kb.id, kb.name, dept_id, kb.retrieval_type)
    return kb


def list_kb(db: Session, user: User) -> list[KnowledgeBase]:
    """知识库列表:super_admin 全部;dept_admin / employee 仅本部门。"""
